...
Это хорошо видно здесь в pair, set и особенно в tuple;
```cpp
float a = 1.0;
int b = 10;
char c = 'v';
std::tuple<float, int, char> tpl(a, b, c);
std::pair<float, int> pair(a, b);
```
мы связываем разные типы данных в одной структуре данных


### constexpr, сравнение с const, enum, макросами.

#### Constexpr #
Скалярный объект, объявленный с помощью спецификатора класса хранения constexpr, является константой. Он должен быть полностью и явно инициализирован в соответствии с правилами статической инициализации. У него все еще есть привязка, соответствующая его объявлению, и он существует во время выполнения, чтобы его адрес был взят; он просто не может быть изменен во время выполнения каким-либо образом, т.е. компилятор может использовать свои знания о фиксированном значении объекта в любом другом константном выражении.
Кроме того, константное выражение, которое используется для инициализации такой константы, проверяется во время компиляции.
Инициализатор типа с плавающей запятой должен быть вычислен в среде с плавающей запятой во время перевода.
Существуют некоторые ограничения на тип объекта, который может быть объявлен с помощью constexpr. А именно, следующие конструкции не могут быть constexpr:

- Указатели (за исключением того, что нулевые указатели могут быть constexpr),
- Изменяемые типы,
- Атомарные типы,
- Изменяемые типы volatile,
- Ограничивающие указатели.

Ключевое слово, указывающее, что значение или функция могут быть вычислены на этапе компиляции.
```cpp
constexpr int SIZE = 100;
constexpr int square(int x) { return x * x; }
```
Короче берем вычисляем и делаем константой во время компиляции


#### const #
неизменяемость во время выполнения
Ключевое слово, указывающее, что значение не может быть изменено после инициализации.

Применение:
```cpp
const int MAX = 100;  // Значение неизменно, но может вычисляться в runtime.
```

Сравнение с constexpr:
- constexpr гарантирует вычисление на этапе компиляции, const — нет.
- constexpr можно использовать для функций и более сложных вычислений.


#### enum #
перечислени
Тип данных, содержащий набор именованных констант.
Виды:
- Обычные (enum):
```cpp
enum Color { RED, GREEN, BLUE };  // RED = 0, GREEN = 1, ...
```
- С областью видимости (enum class):
```cpp
enum class Color { RED, GREEN, BLUE };  // Доступ через Color::RED
```

Преимущества перед макросами:
- Типобезопасность.
- Не загрязняют глобальное пространство имён (enum class)


#### Макросы #
#define
Директивы препроцессора, заменяющие текст до компиляции. Всё происходит на этапе предпроцессинга
...
- Могут быть сложные ошибки из-за подстановки:

```cpp
SQUARE(++x)  // ((++x) * (++x)) — undefined behavior!
```

Использовать только если нет альтернатив (например, условная компиляция #ifdef).

В целом вот таблица сравнений:
| Критерий          | `constexpr`       | `const`          | `enum`/`enum class` | Макросы          |
|-------------------|-------------------|------------------|----------------------|------------------|
| **Время вычисления** | Компиляция       | Runtime/Compile  | Компиляция           | Препроцессор     |
| **Типобезопасность** | Да               | Да               | Да (`enum class`)    | Нет              |
| **Область видимости**| Ограничена       | Ограничена       | Ограничена (`class`) | Глобальная       |
| **Применение**      | Константы, функции | Константы       | Набор значений       | Условная компиляция |


Небольшой вывод:
- Использовать constexpr для вычислений на этапе компиляции.
- const — для констант времени выполнения.
- enum class — для типобезопасных перечислений.
- Макросы — только в крайних случаях (например, для #pragma или условной компиляции).

Пример замены макроса на constexpr:
```cpp
// Было:
#define PI 3.14159
...
#### volantile #
Volatile — это ключевое слово, которое сообщает компилятору, что переменная может изменяться неожиданным образом (например, извне программы), и поэтому её нельзя оптимизировать.

Основное назначение:
- Запрет оптимизаций компилятора
    - Без volatile компилятор может:
        - Заменить обращение к переменной на ранее прочитанное значение (кеширование).
        - Удалить "лишние" чтения/записи, если они кажутся избыточными.

Синтаксис:
```cpp
volatile int x;       // Переменная может измениться неожиданно
volatile int* ptr;    // Указатель на volatile-данные
int* volatile ptr;    // Сам указатель volatile (редко используется)
```

Пример работы volatile и его важность:

- без volatile (проблема оптимизации)
```cpp
int flag = 0;

while (flag == 0) {
    // Ждём, пока флаг изменится (например, из другого потока)
}
```
Компилятор может оптимизировать это в бесконечный цикл, так как он не видит изменений flag внутри цикла.

- с ним (корректная работа)
```cpp
volatile int flag = 0;

while (flag == 0) {
    // Компилятор гарантирует чтение flag из памяти на каждой итерации
}
```
...
#### typedef #
---------------------------
Ключевое слово typedef в C++ используется для создания псевдонимов существующих типов данных, пользовательских типов данных и указателей на более осмысленные имена. Typedef позволяют вам давать описательные имена стандартным типам данных, что также может помочь вам самостоятельно документировать свой код. В основном typedef используются для создания псевдонимов, только если предопределенное имя слишком длинное или сложное для повторного написания. Ненужное использование typedef, как правило, не является хорошей практикой.

Синтаксис: 
``` typedef current_name new_nam ```

Применение typedef в C++:
- typedef в C++ можно использовать для создания псевдонимов предопределенных типов данных с длинными именами.
- Его можно использовать со структурами данных STL, такими как векторы, строки, карты и т. д.
- typedef можно использовать и с массивами.
...
Также можно написать using namespace ... 
и тогда то пространство имен которое указано в ... можно будет опустить в коде 
...
Ключевое auto слово является заполнителем для типа, но оно не является типом. Поэтому ключевое auto слово нельзя использовать в приведениях или операторах, таких как sizeof и (для C++/CLI). Ну это понятно я думаю)
...
- expression Если параметр является rvalue, ```decltype(expression)``` является типом ```expression```. ```expression``` Если параметр является lvalue, decltype(expression) это ссылка на lvalue типаexpression.
...
![image](https://github.com/user-attachments/assets/2410aa35-1cc5-46c3-a600-86c3bb1452ee)
...
person.age = 38;
```
Ну и также мы можем вызывать функции у объекта (Функции в классх называются методами!):
```cpp
person.print();
```
...
➡️ [**Исключения**](➡️ [**Шаблон класса**](https://metanit.com/cpp/tutorial/9.1.php) ) 
