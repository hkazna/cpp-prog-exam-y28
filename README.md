...
| `decltype(a->x);`      | `double`          | Тип члена класса (членского доступа).                                     |
| `decltype((a->x));`    | `const double&`   | Скобки заставляют рассматривать выражение как lvalue, а не член класса.   |
|                        |                   | Поскольку `a` — `const`-указатель, результат — ссылка на `const double`.  |
